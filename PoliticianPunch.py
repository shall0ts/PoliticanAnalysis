...
senators = pandas.read_excel('notebooks\congress_twitter.xlsx')
reps = pandas.read_excel('notebooks\congress_twitter.xlsx', sheet_name=1)
...
def representativeMentionCount(reps, list, start, stop):
    for i in range(start, stop):
        link = reps.at[i, 'Link']
        username = link.split('/')[3]
        mentionCount = client.get_recent_tweets_count(username)
        list.append(mentionCount.meta['total_tweet_count'])
    reps['Mention Totals'] = pandas.Series(list)
...
def main():
    ...
    tempList = []
    representativeMentionCount(reps, tempList, 400, len(reps.index))
    #representativeFollowCount(reps)
    #senators.to_excel('notebooks\outputCount.xlsx', sheet_name = 'Senators')
    reps.to_excel('notebooks\ouputCount.xlsx', sheet_name = 'Reps')
main()
